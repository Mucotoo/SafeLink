var Dosyatc = {
    ...
    checkFileNameFormat: function() {
        if (!Dosyatc.check_file_name_format) {
            return false;
        }
        ...
        JQ("#u_upload_form").find(":file").each(function() {
            if (JQ(this).val() !== "") {
                ...
                if (file_name.length > Dosyatc.max_file_name_chars) {
                    ...
                    Dosyatc.showAlert("Hata, dosya adı çok uzun " + Dosyatc.max_file_name_chars + " characters.", 500, 85, Dosyatc.block_ui_enabled);
                    found_error = true;
                }

                if (file_name.length < Dosyatc.min_file_name_chars) {
                    Dosyatc.highlightFileLabel(JQ(this).attr("id") + "_label", Dosyatc.file_label_highlight_on);
                    Dosyatc.showAlert("Hata, dosya adı çok kısa " + Dosyatc.min_file_name_chars + " characters.", 500, 85, Dosyatc.block_ui_enabled);
                    found_error = true;
                }
    ...
    checkAllowFileExtensions: function() {
        if (!Dosyatc.check_allow_extensions_on_client) {
            return false;
        }
        ...
        JQ("#u_upload_form").find(":file").each(function() {
            if (JQ(this).val() !== "") {
                ...
                if (!file_extension.match(Dosyatc.allow_extensions)) {
                    Dosyatc.highlightFileLabel(JQ(this).attr("id") + "_label", Dosyatc.file_label_highlight_on);
                    Dosyatc.showAlert('"' + file_extension + '" dosya uzantısını yüklemek yasak. Rar veya zip içinde yükleyebilirsiniz.', 500, 85, Dosyatc.block_ui_enabled);
                    found_error = true;
                }
            }
        });
    ...
    checkDisallowFileExtensions: function() {
        if (!Dosyatc.check_disallow_extensions_on_client) {
            return false;
        }
        ...
        JQ("#u_upload_form").find(":file").each(function() {
            if (JQ(this).val() !== "") {
                ...
                if (file_extension.match(Dosyatc.disallow_extensions)) {
                    Dosyatc.highlightFileLabel(JQ(this).attr("id") + "_label", Dosyatc.file_label_highlight_on);
                    Dosyatc.showAlert('Bu dosya uzantısı "' + file_extension + '" yasak. Rar veya zip içinde yükleyebilirsiniz.', 500, 85, Dosyatc.block_ui_enabled);
                    found_error = true;
                }
            }
        });
    ...
    checkNullFileCount: function() {
        if (!Dosyatc.check_null_file_count) {
            return false;
        }
    ...
    checkDuplicateFileCount: function() {
        if (!Dosyatc.check_duplicate_file_count) {
            return false;
        }
        ...
        if (found_duplicate) {
            Dosyatc.showAlert("Aynı dosyalar seçili.", 400, 80, Dosyatc.block_ui_enabled);
            return true;
        } else {
            return false;
        }
    ...
    showCGIOutput: function(CGI_message, reset_page) {
        ...
        if (reset_page) {
            Dosyatc.resetFileUploadPage();
        }
    },

    showDebugMessage: function(message) {
        JQ("#u_debug").append(message + "<br>");
    },

    showAlertMessage: function(message) {
        JQ("#u_alert").html(message);
    },
    ...
    stopDataLoop: function() {
        ...
        if (Dosyatc.cedric_progress_bar) {
            if (Dosyatc.show_current_position) {
    ...
    resetProgressBar: function() {
        ...
        if (Dosyatc.show_percent_complete) {
            JQ("#percent_complete").html("0%");
        }
        if (Dosyatc.show_files_uploaded) {
        ...
        if (Dosyatc.show_elapsed_time) {
            JQ("#elapsed_time").html("00:00:00");
        }
        if (Dosyatc.show_est_time_left) {
            JQ("#est_time_left").html("00:00:00");
        }
        if (Dosyatc.show_est_speed) {
            JQ("#est_speed").html("0");
        }
    },
    ...
    // Link the upload
    linkUpload: function() {
        if (Dosyatc.check_file_name_format || Dosyatc.check_allow_extensions_on_client || Dosyatc.check_disallow_extensions_on_client || Dosyatc.check_duplicate_file_count) {
            Dosyatc.clearFileLabels();
        }
        if (Dosyatc.checkFileNameFormat()) {
            return false;
        }
        if (Dosyatc.checkAllowFileExtensions()) {
            return false;
        }
        if (Dosyatc.checkDisallowFileExtensions()) {
            return false;
        }
        if (Dosyatc.checkNullFileCount()) {
            return false;
        }
        if (Dosyatc.checkDuplicateFileCount()) {
            return false;
        }
    ...
    initializeProgressBar: function(upload_id, debug_ajax) {
        if (debug_ajax) {
            Dosyatc.showDebugMessage("Dosya Yükleniyor: " + Dosyatc.path_to_set_progress_script + '?upload_id=' + upload_id);
        }
    ...
    startUpload: function(upload_id, debug_upload, debug_ajax) {
        ...
        if (debug_ajax) {
            Dosyatc.showDebugMessage("Submitting Upload: " + Dosyatc.path_to_upload_script + "?upload_id=" + upload_id);
        }
        ...
        JQ("#reset_button").val("Yükleme İptal");

        if (!debug_upload) {
            Dosyatc.initializeProgressBar(upload_id, debug_ajax);
        } else {
            Dosyatc.showAlertMessage("Debug Uploader Detected, Please Wait...");
        }
    },
    ...
    startProgressBar: function(upload_id, upload_size, start_time) {
        ...
        if (Dosyatc.show_current_position) {
            JQ("#total_kbytes").html(Dosyatc.total_kbytes + " ");
        }
        if (Dosyatc.show_elapsed_time) {
            Dosyatc.UP_timer = setInterval("Dosyatc.getElapsedTime()", 1000);
        }
    ...
    setProgressStatus: function(total_bytes_read, files_uploaded, current_file, bytes_read, lapsed_time) {
        ...
        if (byte_speed > 0) {
            time_remaining = Math.round((Dosyatc.total_upload_size - total_bytes_read) / byte_speed);
        }
        ...
        if (Dosyatc.cedric_progress_bar === 1) {
            if (byte_speed !== 0) {
                ...
                if (temp_CPB_time_width < 5001) {
                    Dosyatc.CPB_time_width = temp_CPB_time_width;
                }
                if (temp_CPB_time_bytes < 5001) {
                    Dosyatc.CPB_time_bytes = temp_CPB_time_bytes;
                }
            } else {
        ...
        if (Dosyatc.cedric_progress_bar === 1) {
            if (Dosyatc.cedric_hold_to_sync) {
                if (progress_bar_status < Dosyatc.CPB_width) {
                    Dosyatc.CPB_hold = true;
                } else {
            ...
            if ((Dosyatc.BPB_width_inc < Dosyatc.BPB_width_old) && (Dosyatc.BPB_width_new > Dosyatc.BPB_width_old)) {
                Dosyatc.BPB_width_inc = Dosyatc.BPB_width_old;
            }
    ...
    addUploadSlot: function() {
        if (JQ(".upfile_ultimo").val() !== "") {
            if (JQ(".upfile").length < Dosyatc.max_upload_slots + 1) {
                ...
                if (JQ(".upfile").length > Dosyatc.max_upload_slots) {
                    JQ(".upfile_ultimo").fadeOut("fast");
                }
            }
        }
